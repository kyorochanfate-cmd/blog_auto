...
【商品カード(画像+Amazon/楽天ボタン) — 本文で実際に論じた製品だけを紹介する】
...
**絶対のルール — 無関係な商品を紹介しない**:
- カードにして良いのは **その段落で名前を出して実際に論じた製品のみ**。
  「関連商品」「話のついでに売れそうなもの」を後付けで探して挿入するのは禁止。
- 主題そのものが Amazon/楽天で買えない場合(ロボット・AIサービス・政策・イベント企画・
  自作/一点物のガジェットなど)は、**無理に代替商品を持ち出さない。カード0個で記事を完結させる**。
  例:「デジタルサイネージ搭載ガチャマシン」の記事で AR グラスや防犯カメラを
  勧めるのは、話題が繋がっていないので**禁止**。
- 比較記事で競合機種に触れているなら、その競合機種はカードにしてよい(本文で論じているため)。
- 本体の話をしている記事にアクセサリだけ紹介するのも避ける。アクセサリに触れるなら
  本文中でそのアクセサリについて具体的に書いてからカードにする。

**具体例 (いずれも「本文でその製品を論じた」場合のみ)**:
...
- **型番だけ・ブランド名を省略したカードは禁止**(例: `DR02` ✗ → `〇〇メーカー DR02` ✓)。
  型番だけだと、たまたま同じ型番の全く無関係な他社製品(別ジャンル)がヒットしてしまう
- カードは、その製品について論じた段落の直後の独立した行に置く(前後に空行)
- 「## 関連商品」「## こちらもおすすめ」のような、本文の話題から切り離した紹介セクションは作らない
...
**プレースホルダーが楽天で見つからなければ自動的に削除される**。
無理に個数を稼ぐ必要はない。0個・1個で記事が完結するならそれでよい。
"""
